...
export default class CookieStore {
  jar = CookieStore.createSubdomainStore('.', '.')
  ...
  #set(cookie, domain, path) {
    ...
    for(let i = domainSplit.length-1; i >= itstop; i--) {
      ...
      if(!jar.sub[subdomain]) jar.sub[subdomain] = CookieStore.createSubdomainStore(subdomain, cookie.domain)
  ...
  static createSubdomainStore = (domain = '.', subdomain = '.') => {
    return {
      sub: {},
      multi: new Cookies,
      local: new Cookies,
